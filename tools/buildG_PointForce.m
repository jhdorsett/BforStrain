function [Ge_x, Ge_y, Gn_x, Gn_y,GExx_x,GExy_x,GEyy_x,GExx_y,GExy_y,GEyy_y] = buildG_PointForce(xy_gps,nodes,nu)
...
for k=1:size(nodes,1)
    ...
    [Ge_x(:,k), Ge_y(:,k), Gn_x(:,k), Gn_y(:,k), GExx_x(:,k),GExy_x(:,k),GEyy_x(:,k),GExx_y(:,k),GExy_y(:,k),GEyy_y(:,k)] = PointForce(nodes(k,1),nodes(k,2),xy_gps(:,1),xy_gps(:,2),nu);
